...
# Methods
As stated in the First Law of Geography, "Everything is related to everything else, but near things are more related than distant things" [**(Tobler, 1970)**](), and since depopulation is a variable with spatial dependence, to carry out our purpose we deal with geostatistics and machine learning tecniques. 
Figure \@ref(fig:fig1) stands for the methodology used in this work:
...
1. The instrument used *par excellence* to $\color{#FFC000}{\text{detect the spatial dependence}}$ in a regionalized variable [**(Montero et al., 2015)**]() is the semivariogram. Its expression is given by:
\begin{equation}
\tag{1}
...
3. Based on the range, $\color{#453780}{\text{a neighbourhood network}}$ is constructed in the form of a proximity matrix $L$. 
...
5. $\color{#288A8C}{\text{The extraction of the first principal component}}$ is carry out.

6. The sDRI for each municipality is obtained $\color{#288A8C}{\text{scaling from 0 to 100}}$ the principal component.
:::
...
In the opposite, we have Arandilla del Arroyo (sDRI = 100), followed by Alique, Valsalobre, Angón and Pineda de Cigüela. Figure \@ref(fig:fig6)[-right]() represents the sDRI in a map of municipalities of Castilla-La Mancha.
:::
:::
