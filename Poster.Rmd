...
```{r, fig1, out.width="90%", fig.align='center', fig.cap="Components of a semivariogram"}
knitr::include_graphics("PNG/Fig 1 - Semivariogram.png")
```
...
Figure \@ref(fig:fig2) shows the extreme theoretical posibilities. The last step is extracting and scaling the first principal component of the sPCA.

```{r, fig2, fig.align = 'center', fig.cap = "Theoretical cases: (a) spatial dependence, (b) no spatial dependence", fig.id = TRUE}
knitr::include_graphics("PNG/Fig 2 - sACP.png")
```
:::
...
::: columns
::: {.column width="30%"}
Figure \@ref(fig:fig3) shows the adjusted semivariogram to a spherical model, resulting in a range of 60 km for the spatial dependence, a sill of 3419 and a nugget of 1667.

Therefore, the calculated range of spatial dependence is of around one-sixth of the regional limits.

```{r, fig3, fig.align = 'center', fig.cap = "Semivariogram", fig.id = TRUE}
knitr::include_graphics("PNG/Fig 3 - Semivariogram.png")
```
:::
...
::: {.column width="68%"}
```{r, fig4,out.width = "90%", fig.align = 'center', fig.cap = "Principal results of spatial principal component analysis (range: 60km): (a) Eigenvalues of sPCA; (b) Map of sPCA scores of municipalities.", fig.id = TRUE}
knitr::include_graphics("PNG/Fig 4 - sPCA.png")
```
Figure \@ref(fig:fig4) shows the main results of the spatial analysis of principal components of depopulation in Castilla-La Mancha. The first two eigenvalues of sPCA (\@ref(fig:fig4)[a]()) show a strong global spatial dependence, whereas the last negatives eigenvalues reveal some local dependence; this is due to municipalities acting as development hubs, consequently earning population of neighbours. The sPCA map (\@ref(fig:fig4)[b]()) shows three big areas of depopulation; namely the counties of Cuenca and Guadalajara, the west and the south of the region.
:::
.
...
Finally, the sDRI (\@ref(fig:fig5)) is extracted from the first component of sPCA and scaled from 0 to 100. Municipalities are then classified from Albacete (sDRI = 0) to Arandilla del Arroyo (sDRI = 100). \@ref(fig:fig6) represents the sDRI in a map of Castilla-La Mancha.

::: columns
::: {.column width="34%"}
```{r, fig5, fig.align='center', fig.cap="Depopulation Risk in municipalities of Castilla-La Mancha according to sDRI Indicator"}
knitr::include_graphics("PNG/Fig 5 - Lollipop.png")
```
:::

::: {.column width="66%"}
```{r, fig6, fig.align='center', fig.cap="Depopulation Risk in municipalities of Castilla-La Mancha according to sDRI Indicator"}
knitr::include_graphics("PNG/Fig 6 - sDRI.png")
```
:::
:::
:::
